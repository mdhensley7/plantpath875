...
genes_dir <- "~/Desktop/phylo-practicum/glemin-wheat/data/Wheat_Relative_History_Data_Glemin_et_al/OneCopyGenes/"
concat_10_dir <-"~/Desktop/phylo-practicum/glemin-wheat/data/Wheat_Relative_History_Data_Glemin_et_al/OneCopyGenes-10mb/"

dir.create(concat_10_dir)

gene_map_loc <- "~/Desktop/phylo-practicum/glemin-wheat/data/Wheat_Relative_History_Data_Glemin_et_al/MappedOnHordeumChromosomes_OneCopyGenes.txt"
...
for(chromosome in unique(gene_map$Chrom)){
  ...
  # The line of code below is super powerful as it prints us an idea of how many genes are found in each window
  # on each chromosome. The outputs gives us an idea of gene dense regions of each chromosome
  print(paste("we aare in chrom",chromosome, "at pos", start_pos,'it has genes:',length(window_genes)))
    
# command + shift + C will add a hashtag to any line you have highlighted and this comments out a chunk of 
# code so it will not run (example below)
